...
public class InitApplication {
    ...
    public static void onApplicationCreate(Application application) {
        String managerLocation = application.getExternalFilesDir("shadow") + "/sample-manager-debug.apk";

        //Log接口Manager也需要使用，所以主进程也初始化。
        LoggerFactory.setILoggerFactory(new AndroidLoggerFactory());

        if (isPluginProcess(application)) {
        ...
        if (isProcess(application, application.getPackageName())) {
            FixedPathPmUpdater fixedPathPmUpdater
                    = new FixedPathPmUpdater(new File(managerLocation));
    ...
    private static boolean isPluginProcess(Application application) {
        return isProcess(application, ":plugin")||isProcess(application, ":plugin.kyxlstu")||isProcess(application, ":plugin.kyxltea");
    }
